...
\documentclass[25pt, a0paper, portrait, margin=12mm, innermargin=14mm,
               blockverticalspace=14mm, colspace=14mm, subcolspace=8mm]{tikzposter}
...
\block{1\,\textbar\, Motivation \& Research Questions}{
LLMs deliver state-of-the-art reasoning at heavy \textbf{latency, cost, and energy}
overheads; collaborative SLM--LLM architectures promise near-LLM quality at a fraction of
the resources. A companion \textbf{PRISMA 2020 systematic review of 54 studies} found that
\textbf{cost and energy are routinely underreported} and that no study compares multiple
architecture families under one shared measurement policy.

\vspace{8pt}
\innerblock{Research Questions}{%
\begin{itemize}
  \item \textbf{RQ1 Performance:} can collaborative architectures match standalone LLM accuracy?
  \item \textbf{RQ2 Efficiency:} what are the cost / latency / energy trade-offs across families?
  \item \textbf{RQ3 Unified metric:} does EATS give a stable architecture-agnostic ranking?
  \item \textbf{RQ4 Grounding:} do measured results corroborate the literature-review trends?
\end{itemize}}
}
...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, rounded corners=6pt, draw=iyteDark!40, line width=1.2pt,
              minimum width=0.78\linewidth, minimum height=1.65cm, align=center,
              inner sep=8pt, font=\normalsize},
...
\node[box, fill=accentGold!18, below=1.0cm of arch] (proxy) {\textbf{Metrics Proxy + vLLM}\\
...
Accuracy $\alpha$ is combined with cost, latency, and energy ratios, each normalised by the
...
\end{align*}
\begin{itemize}
  \item Strictly increasing in accuracy, decreasing in resource penalty.
  \item Baseline reduces to $\alpha/(\alpha+1)$ --- a clean reference curve.
  \item Weights reflect deployment priorities: cost $>$ latency $>$ energy.
\end{itemize}
}

\block{4\,\textbar\, Energy, Carbon \& Cost Measurement}{
A metrics proxy on each GPU host measures inference time and samples GPU power via NVIDIA
NVML (CodeCarbon preferred), so the measurement boundary sits \emph{at the inference host}
--- real hardware power draw, not network or orchestration overhead.
\begin{align*}
  ...
  C = C_{\text{API}} + r t
\end{align*}
Grid intensity $k=380$ gCO$_2$/kWh; hourly rates: L4 \$0.75 $\cdot$ RTX PRO6000 \$2.35
$\cdot$ H200 \$6.50.
}

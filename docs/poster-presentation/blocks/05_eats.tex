\begin{block}{EATS: Efficiency-Aware Task Score}
  ...
  \begin{tikzpicture}
    \node[draw=posterTeal, fill=posterTeal!12, rounded corners=6pt, inner sep=12pt,
      ...
      \[
        ...
        \frac{\text{accuracy}^2}
             {\text{accuracy}^2 +
              \text{normalized\_cost}^{0.5}
              \text{normalized\_algorithmic\_latency}^{0.3}
              \text{normalized\_energy}^{0.2}}
      \]
    };
  \end{tikzpicture}
  \end{center}
  ...
  \end{itemize}
  Higher is better: the squared accuracy term keeps answer quality primary,
  while the multiplicative penalty suppresses systems that are costly, slow, or
  energy-hungry. The score is bounded to $[0,1]$.

...
EATS is still useful in this table, but it should be read carefully. The
single-SLM baseline reaches an EATS of 83.0 because the current formula applies
a special-case reward when the LLM-call ratio is zero. That makes the value
structurally inflated relative to mixed or always-LLM systems. For this reason,
EATS in the GSM8K comparison should be treated as an efficiency signal rather
than as a standalone quality ranking. The more reliable interpretation comes
from reading EATS together with accuracy, latency, cost, and resource metrics.

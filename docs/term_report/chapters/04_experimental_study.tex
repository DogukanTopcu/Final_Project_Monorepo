...
EATS of 0.325, while the monolithic baseline reached 0.419. Under the current
bounded accuracy-first formulation, routing no longer wins the primary score
because its large latency penalty outweighs its small cost and energy savings.
The main driver is selective escalation overhead: 66 samples were accepted at
the SLM stage, but 34 required the heavy LLM and every routing request still
paid for an initial SLM pass.
...
Monolithic & \metricbar{monolithiccolor}{4.00cm} & 0.419 \\
\end{tabular}
\end{minipage}
...
EATS is still useful in this table, but it now behaves much more reasonably.
...
accuracy-squared versus efficiency-penalty formulation. The monolithic LLM

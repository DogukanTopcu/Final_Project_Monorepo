...
Third, three architectures from the Chapter~3 taxonomy---Debate (POA), Blackboard, and Entropy Blackboard---have not been benchmarked. The current evaluation therefore does not yet cover the full architecture space defined by the present study.
...
\begin{enumerate}
    ...
    \item \textbf{Remaining architecture benchmarking.} Debate (POA), Blackboard, and Entropy Blackboard are implemented in the repository but have not been evaluated. Benchmarking these architectures on the same five benchmarks (MMLU, ARC-Challenge, GSM8K, HellaSwag, and TruthfulQA) will complete the taxonomy coverage and provide empirical data for the full set of architectures introduced in Chapter~3.

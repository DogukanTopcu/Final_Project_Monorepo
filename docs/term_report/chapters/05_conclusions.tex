...
\textbf{RQ1 (Performance):} Routing and speculative decoding match or approach the accuracy of the best standalone models on ARC, HellaSwag, and TruthfulQA. Pure Swarm exceeds every standalone model on ARC. The answer to RQ1 is affirmative for these benchmarks: collaborative architectures can match standalone LLM accuracy. On MMLU, Pure Swarm reaches the highest reported accuracy, while routing and speculative decoding remain competitive but do not exceed the top standalone or swarm configurations.
...
\begin{enumerate}
    \item \textbf{GSM8K output-parsing fix.} The numeric answer extraction heuristic should be audited and corrected for GPT-family output formats. The GPT-family GSM8K runs should then be re-executed to verify whether the 67.8\%/59.4\% anomaly was a parsing artefact.

    \item \textbf{Remaining architecture benchmarking.} Debate (POA), Blackboard, and Entropy Blackboard architectures are implemented in the repository but have not been evaluated. Benchmarking these families on the same five benchmarks (MMLU, ARC-Challenge, GSM8K, HellaSwag, and TruthfulQA) will complete the taxonomy coverage and provide empirical data for the full set of architecture families introduced in Chapter~3.

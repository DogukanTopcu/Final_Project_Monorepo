...
from web.backend.services import gcp_service
...
async def list_instances():
    try:
        return gcp_service.list_instances()
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"GCP error: {e}")


@router.post("/infrastructure/instances/{instance_id}/start")
async def start_instance(instance_id: str):
    try:
        result = gcp_service.start_instance(instance_id)
...
async def stop_instance(instance_id: str):
    try:
        result = gcp_service.stop_instance(instance_id)
        return {"instance_id": instance_id, "state": result}
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"GCP error: {e}")
...
async def get_costs():
    try:
        return gcp_service.get_cost_estimate()
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"GCP error: {e}")
